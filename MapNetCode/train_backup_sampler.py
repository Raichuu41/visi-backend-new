...
def mutual_k_nearest_neighbors(vectors, sample_idcs, k=1):
    ...
    _, idx = index.search(vectors[sample_idcs], len(vectors))
...
def train(net, feature, image_id, old_embedding, target_embedding, idx_modified, idx_old_neighbors, idx_new_neighbors,
    ...
    sample_loader = DataLoader(IndexDataset(feature), batch_size=batch_size,
                               sampler=torch.utils.data.SubsetRandomSampler(idx_modified), **kwargs)

    neighbor_loader = DataLoader(IndexDataset(feature), batch_size=batch_size,
                                 sampler=torch.utils.data.SubsetRandomSampler(neighbors), **kwargs)

    space_loader = DataLoader(IndexDataset(feature), batch_size=2*batch_size,
                              sampler=torch.utils.data.SubsetRandomSampler(space_samples),
                              drop_last=True, **kwargs)

    fixpoint_loader = DataLoader(IndexDataset(feature), batch_size=batch_size,
                                 sampler=torch.utils.data.SubsetRandomSampler(previously_modified), **kwargs)
    ...
    while not stop_criterion:
        ...
        for batch_idx, (space_data, space_indices) in enumerate(space_loader):
            ...
            if len(fixpoint_loader) == 0:
                ...
                data = torch.cat([space_data, sample_data, neighbor_data, fixpoint_data])
            indices = torch.cat([space_indices, sample_indices, neighbor_indices, fixpoint_indices])
            ...
            idx_l2_fixed = torch.cat([space_indices, sample_indices, fixpoint_indices])
            l2_loss = torch.mean(
            ...
            n_tot = len(l2_loss)
            n_space, n_sample, n_fixpoint = len(space_indices), len(sample_indices), len(fixpoint_indices)
            l2_loss = 0.8 * torch.mean(l2_loss[:n_space]) + 0.2 * torch.mean(l2_loss[n_space:])
            ...
            if epoch > 5 and batch_idx >= 2 * len(neighbor_loader):
                print('\tend epoch after {} random fix point samples'.format((batch_idx+1) * space_loader.batch_size))
                break
