...
def write_config(args, exp_name, defaults=None, extras=None):
    ...
    if len(to_file):
        with open(exp_name + '_config.txt', 'w') as f:
...
def load_weights(weightfile, state_dict_model, prefix_file='', prefix_model=''):
    ...
    for k in set(list(state_dict_model.keys()) + list(pretrained_dict.keys())):
...
def find_stop_accuracy(accuracy_threshold, bool_list):
    """Returns last index for desired acc."""
    acc_cum = np.cumsum(bool_list) / (np.arange(bool_list.shape[0]) + 1)
    if np.any(acc_cum > accuracy_threshold):
        idx_stop = acc_cum.shape[0] - np.where(acc_cum[-1::-1] >= accuracy_threshold)[0][0] -1
    else:
        idx_stop = 0
    return idx_stop
