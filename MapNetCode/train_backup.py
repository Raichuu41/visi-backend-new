...
def svm_k_nearest_neighbors(vectors, sample_idcs, negative_idcs=None, k=1):
    ...
    if len(negative_idcs) == 0:
        ...
        sample_weights = np.concatenate([5 * np.ones(len(sample_idcs)), np.ones(N_unlabeled)])
    else:
...
def select_neighbors(feature, sample_idcs, negative_idcs=None, k=1,
    ...
    neighbors, scores = neighbor_fn(feature, sample_idcs, negative_idcs=negative_idcs, k=k)
...
def train(net, feature, image_id, old_embedding, target_embedding,
    ...
    while not stop_criterion:
        ...
        for batch_idx in range(n_batches):
            ...
            idx_l2_fixed = np.concatenate([new_neigh_indices, moved_indices, negatives_indices, prev_moved_indices,
                                           fixed_indices]).astype(long)
            l2_loss = torch.mean(
                l2_criterion(emb_mod[:n_new + n_moved + n_neg + n_prev + n_fixed],
            ...
            if epoch < 0:
                ...
                distance_loss_input = fts_mod[:-(n_old + n_high)] if (n_old + n_high) > 0 else fts_mod
                distance_loss_target = torch.cat([torch.ones(n_new + n_moved), torch.zeros(n_neg + n_prev + n_fixed)])
                distance_loss_weights = torch.cat([torch.ones(n_new+n_moved+n_neg+n_prev), 0.5 * torch.ones(n_fixed)])
                ...
                if n_neg > 0:
                    selected_negatives = {1: np.arange(n_new + n_moved, n_new + n_moved + n_neg)}
                else:
                    selected_negatives = None

                distance_loss, negative_triplets = distance_criterion(distance_loss_input, distance_loss_target, concealed_classes=[0], weights=distance_loss_weights, selected_negatives=selected_negatives)
                if negative_triplets is not None:
                    negative_triplets = np.unique(negative_triplets.numpy())
                    negative_triplets = indices[:-(n_old + n_high)][negative_triplets]
