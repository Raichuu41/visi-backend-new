...
class ExemplarTripletSelector(TripletSelector):
    def __init__(self, margin, negative_selection_fn, avg_samples_per_cluster, negative_labels=(), niter=20, gpu=True, parallel=True):
    ...
    def get_exemplars(self, feature, label, weights=None):
        ...
        if self.parallel:
            ...
            pool = multiprocessing.Pool(min(num_cores, len(labelset)))
            cluster = zip(*pool.map(partial(get_cluster_indices, **kwargs), labelset))
